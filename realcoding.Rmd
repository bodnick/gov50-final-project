...
wins_barchart_1 <- ggplot(games, aes(x = factor(HOME_TEAM_WINS))) +
  ...
  labs(title = "Win frequency in the NBA, 2003-2019", y = "Count") +
...
pts_linechart <- ggplot(na.omit(games[c("PTS_away", "PTS_home")]), aes(x = PTS_away)) +
  ...
  labs(title = "Points scored in the NBA, 2003-2019", x = "Points", y = "Density") +
  scale_fill_manual(values = c("PTS_away" = "#e74c3c", "PTS_home" = "#3498db"), 
                    labels = c("By visiting teams", "By home teams")) +
  theme_minimal() +
...
```{r}
wins_simplified_table <- data.frame(
  Team = c("Visiting Team", "Home Team"),
  WinRate = c(0.40571, 0.59428)
)

wins_regression_barchart <- ggplot(wins_simplified_table, aes(x = "", y = WinRate, fill = Team)) +
  geom_bar(stat = "identity") +
  labs(title = "Win rates in the NBA, 2003-2019",
       y = "Win rate") +
  ...
  annotate("text", x = 1, y = max(wins_simplified_table$WinRate) * 1.15, 
           label = "Home team = 59.43%", color = "#000000") +
  annotate("text", x = 1, y = max(wins_simplified_table$WinRate) * 0.3,
           label = "Visiting team = 40.57%", color = "#000000")
...
pts_regression_boxplot <- ggplot(pts_long, 
  ...
  ggtitle("Points scored in the NBA, 2003-2019") +
...
wins_season_fit_table <- modelsummary::modelsummary(wins_season_fit,
...
pts_season_fit_table <- modelsummary::modelsummary(pts_season_fit,
                           statistic = c("s.e. = {std.error}",
                                         "p = {p.value}"),
                           gof_map = c("nobs", "r.squared", "adj.r.squared"))
```
...
wins_playoffs_fit_table <- modelsummary::modelsummary(wins_playoffs_fit,
                           statistic = c("s.e. = {std.error}",
                                         "p = {p.value}"),
                           gof_map = c("nobs", "r.squared", "adj.r.squared"))
...
glance(wins_playoffs_fit)
pts_playoffs_fit_table <- modelsummary::modelsummary(pts_playoffs_fit,
                           statistic = c("s.e. = {std.error}",
                                         "p = {p.value}"),
                           gof_map = c("nobs", "r.squared", "adj.r.squared"))
```
