...
wins_long_season <- games |>
  ...
  select(Home, Season, Win)

wins_multiple_fit <- lm(Win ~ Home + Season, data = wins_long_season)
...
pts_long_season <- games |>
  select(SEASON, PTS_home, PTS_away) |> 
  na.omit() |>
  pivot_longer(
...
pts_multiple_fit <- lm(PTS ~ Home + SEASON, data = pts_long_season)
