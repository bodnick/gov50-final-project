...
Here's my first data set, which includes statistics on final scores and free throw percentages. 
...
```{r wins-plot, echo=FALSE, fig.path="images/"}
print(wins_barplot)
print(points_barplot)
print(ft_barplot)
```


In addition to these metrics, I want to look at personal fouls called on each team, which is in my second data set:

```{r, echo=FALSE}
fouls
```

I'll have to do a lot of data cleaning to get that statistic in the form I need, which I will do in the future (or potentially decide not to focus on this additional metric).
